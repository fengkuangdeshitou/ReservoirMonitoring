...
- (IBAction)help:(id)sender{
    if (self.indexPath.section == 0) {
        [GlobelDescAlertView showAlertViewWithTitle:self.titleLabel.text desc:@"During this mode, batteries are discharged to power your home. In self-consumption mode, the user can set the lowest threshold for the Reserve SOC value in order to save some energy for emergency use only." btnTitle:nil completion:nil];
    }else if (self.indexPath.section == 1){
        [GlobelDescAlertView showAlertViewWithTitle:self.titleLabel.text desc:@"During this mode, energy stored by the battery modules is reserved for back up only, and only get discharged in case of grid blackout or other power failures." btnTitle:nil completion:nil];
    }else{
